        ...
        if( gamesettings.thirdPersonView == 0 )
        {
            ...
            else
            {
                ...
                if ( mc.gameSettings.attackIndicator == 1 )
                {
                    ...
                    // if ( ConfigurationHandler.isItemAttackUsable(heldItemOH.getItem()) )
                    {
                        // int cooldown = Helpers.getOffhandCooldown(mc.player);
                        
                        float ohCooldown = Helpers.execNullable(mc.player.getCapability(EventHandlers.TUTO_CAP, null), CapabilityOffhandCooldown::getOffhandCooldown, 0) / (float) EventHandlersClient.offcd;
